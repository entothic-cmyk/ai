...
app.post('/api/auth/register', async (req, res) => {
  ...
  res.cookie(COOKIE, newSession(uid), {
...
app.post('/api/auth/login', async (req, res) => {
  ...
  res.cookie(COOKIE, newSession(user.id), {
    httpOnly: true, sameSite: 'lax', maxAge: 1000 * 60 * 60 * 24 * 30
  });
...
app.get('/api/chats/:id', auth, (req, res) => {
  ...
  const msgs = db.prepare(
    'SELECT role,content,attachments FROM messages WHERE chat_id=? ORDER BY id ASC'
  ).all(req.params.id);
...
app.post('/api/chat', auth, async (req, res) => {
  try {
    ...
    if (!upstream.ok) {
      ...
      res.write(`data: ${JSON.stringify({ error: `Upstream API error (${upstream.status})` })}\n\n`);
      return res.end();
    }
    ...
    // Persist
    if (chatId && fullReply) {
      const chat = db.prepare('SELECT id,title FROM chats WHERE id=? AND user_id=?')
        .get(chatId, req.user.id);
      if (chat) {
        const lastUser = [...messages].reverse().find(m => m.role === 'user');
        db.prepare('INSERT INTO messages (chat_id,role,content,attachments,created_at) VALUES (?,?,?,?,?)')
          .run(chatId, 'user', lastUser?.content || '', JSON.stringify(lastUser?.attachments || []), Date.now());
        db.prepare('INSERT INTO messages (chat_id,role,content,attachments,created_at) VALUES (?,?,?,?,?)')
...
app.get('/chat.html', (req, res, next) => {
  if (!getUser(req)) return res.redirect('/auth.html');
  next();
});
