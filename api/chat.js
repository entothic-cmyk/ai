  ...
  try {
    ...
    if (chatId && fullReply) {
      const chats = await sql`
        SELECT id, title FROM chats WHERE id = ${chatId} AND user_id = ${user.id} LIMIT 1
      `;
      if (chats.length) {
        ...
        await sql`
          ...
          VALUES (${chatId}, 'user', ${lastUser?.content || ''}, ${JSON.stringify(lastUser?.attachments || [])}, ${now})
        `;
        await sql`
          INSERT INTO messages (chat_id, role, content, attachments, created_at)
          VALUES (${chatId}, 'assistant', ${fullReply}, NULL, ${Date.now()})
        `;

        let title = chats[0].title;
        if (!title || title === 'New chat') title = (lastUser?.content || 'New chat').slice(0, 40);

        await sql`UPDATE chats SET title = ${title}, updated_at = ${Date.now()} WHERE id = ${chatId}`;
      }
    }
  } catch (e) {
